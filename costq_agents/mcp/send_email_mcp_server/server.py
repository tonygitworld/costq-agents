"""Send Email MCP Server - 邮件发送服务

专注于邮件发送功能，使用 AWS SES（Simple Email Service）。

设计理念：
    - 单一职责：只负责邮件发送
    - 通用性强：任何场景都可使用（告警、通知、验证码等）
    - 无状态设计：不依赖数据库，每次调用独立
    - 简单纯粹：参数简洁，功能专注
"""

import logging

logger = logging.getLogger(__name__)
from mcp.server.fastmcp import FastMCP

from .handlers.email_handler import send_email

# 配置日志

# Server 说明文档
SERVER_INSTRUCTIONS = """
# Send Email MCP Server - 邮件发送服务

## 🎯 核心功能

专注于邮件发送，使用 AWS SES（Simple Email Service）。

**特性：**
- ✅ 发送 HTML/纯文本邮件
- ✅ 支持多个收件人
- ✅ 自动重试机制（最多3次）
- ✅ 详细的发送日志
- ✅ 完善的错误处理

## 🔧 工具

### send_email - 发送邮件

发送邮件到指定收件人，支持 HTML 和纯文本格式。

**参数：**
- `to_emails` (List[str], 必需): 收件人邮箱列表
- `subject` (str, 必需): 邮件主题
- `body_html` (str, 可选): HTML格式邮件正文
- `body_text` (str, 可选): 纯文本格式邮件正文

**注意：** `body_html` 和 `body_text` 至少提供一个。

**返回值：**
```json
{
  "success": true,
  "message_id": "010101234567890-abcdef...",
  "to_emails": ["user@example.com"]
}
```

**失败时返回：**
```json
{
  "success": false,
  "error": "错误信息",
  "to_emails": ["user@example.com"]
}
```

**使用示例：**

1. 发送简单的 HTML 邮件：
```json
{
  "to_emails": ["user@example.com"],
  "subject": "欢迎使用 CostQ",
  "body_html": "<h1>欢迎</h1><p>感谢您使用 CostQ 成本管理平台。</p>"
}
```

2. 发送纯文本邮件：
```json
{
  "to_emails": ["user@example.com"],
  "subject": "密码重置",
  "body_text": "您的密码重置验证码是：123456"
}
```

3. 同时发送 HTML 和纯文本（推荐）：
```json
{
  "to_emails": ["user@example.com", "admin@example.com"],
  "subject": "AWS 成本告警",
  "body_html": "<h2>成本告警</h2><p>您的 AWS 成本已超过预算。</p>",
  "body_text": "成本告警\\n\\n您的 AWS 成本已超过预算。"
}
```

## 📝 配置说明

### 环境变量

- `SES_REGION`: SES 服务区域（默认：ap-northeast-1）
- `SES_SENDER_EMAIL`: 发件人邮箱（默认：no_reply@costq-mail.cloudminos.jp）
- `SES_CONFIGURATION_SET`: SES 配置集（可选）
- `LOG_LEVEL`: 日志级别（默认：WARNING）

### AWS 权限要求

需要以下 SES 权限：
- `ses:SendEmail`
- `ses:SendRawEmail`

**权限来源：**
- 本地开发：使用 `AWS_PROFILE` 环境变量
- 生产环境：使用 IAM Role（EKS Pod ServiceAccount）

## ⚠️ 注意事项

1. **发件人邮箱**
   - 默认发件人：no_reply@costq-mail.cloudminos.jp
   - 发件人邮箱必须在 SES 中验证

2. **沙盒模式**
   - 新 AWS 账号默认处于沙盒模式
   - 沙盒模式下，收件人邮箱也需要验证
   - 生产环境需要申请移出沙盒

3. **发送限制**
   - AWS SES 有发送速率限制
   - 默认限制：1 封/秒，200 封/天
   - 可以申请提高限制

4. **重试机制**
   - 自动重试最多 3 次
   - 某些错误不会重试（如邮箱未验证）

5. **邮件格式**
   - 推荐同时提供 HTML 和纯文本
   - HTML 邮件需要使用内联 CSS
   - 避免使用外部图片（可能被过滤）

## 🔗 相关 MCP

- **Alert MCP**: 告警管理服务（告警触发后调用本服务发送邮件）
- **其他服务**: 任何需要发送邮件的服务都可以使用

## 💡 最佳实践

1. **邮件主题**
   - 简洁明确
   - 包含关键信息
   - 示例："AWS 成本告警 - 超出预算 50%"

2. **邮件内容**
   - 提供 HTML 和纯文本两个版本
   - HTML 使用简单清晰的结构
   - 包含必要的信息和行动建议

3. **收件人管理**
   - 验证邮箱格式
   - 避免发送到无效邮箱
   - 批量发送时注意速率限制

4. **错误处理**
   - 检查返回的 success 状态
   - 记录失败原因
   - 对于重要邮件，实现补偿机制
"""

# 初始化 FastMCP
mcp = FastMCP("send-email", instructions=SERVER_INSTRUCTIONS)


@mcp.tool()
async def send_email_tool(
    to_emails: list[str], subject: str, body_html: str = "", body_text: str = ""
) -> dict:
    """发送邮件

    Args:
        to_emails: 收件人邮箱列表
        subject: 邮件主题
        body_html: HTML邮件正文（可选）
        body_text: 纯文本邮件正文（可选）

    Returns:
        dict: 发送结果
        {
            'success': True/False,
            'message_id': 'ses-message-id',
            'to_emails': ['user@example.com'],
            'error': 'error message'  # 失败时返回
        }
    """
    return await send_email(
        to_emails=to_emails, subject=subject, body_html=body_html, body_text=body_text
    )


if __name__ == "__main__":
    mcp.run()
