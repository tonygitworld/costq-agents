...
class TestCUDHandlers:
    ...
    async def test_error_handling_no_bigquery_export(self, mock_context, test_project_id):
        ...
        from costq_agents.mcp.gcp_cost_mcp_server.handlers.cud_handler import get_cud_utilization

        # 模拟 BigQuery 表名为 None（未配置）
        with patch(
            "costq_agents.mcp.gcp_cost_mcp_server.handlers.cud_handler.get_gcp_credentials_provider"
        ) as mock_provider:
...
class TestCUDUsageExamples:
    ...
    async def example_identify_optimization_opportunities(self):
        """示例: 识别优化机会"""
        from costq_agents.mcp.gcp_cost_mcp_server.handlers.cud_handler import get_cud_coverage
